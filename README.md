...
## 📌 Remaining / Next Steps for Development

1. **Authentication & Authorization System**:
   * You already have `User` and `RefreshToken` models and JWT settings defined.
   * Next step: Implement password hashing (e.g. `passlib` or `pwdlib` / `bcrypt`), JWT token generation, and `/api/v1/auth/register` and `/api/v1/auth/login` endpoints.
2. **CRUD Endpoints for Core Entities**:
   * Create routes and schemas in `app/api/routes/` for `skills`, `jobs`, `courses`, and `districts`.
3. **Embeddings & Vector Search (pgvector)**:
   * When jobs and skills are ingested, generate embeddings (1536-dimensional vectors) and use pgvector's distance operators (e.g., `<=>` cosine distance) to perform semantic matching between candidate skills and job requirements.
4. **Seed Data Script**:
   * Add a script in `scripts/seed_data.py` to populate initial sectors, districts, and NSQF qualifications.
